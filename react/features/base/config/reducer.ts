 ...
 * JitsiMeetJS#init(). The app will download config.js from the grommunio meet
 ...
 * The app will download config.js from the grommunio meet deployment and take its
 ...
 * config.js from the grommunio meet deployment and take its values into account but
...
function _setConfig(state: IConfig, { config }: { config: IConfig; }) {
    ...
    const newState = _.merge(
        ...
        // downloaded from the grommunio meet deployment because the former contains
 ...
 * interface_config {@code Object} which is in the latest format supported by grommunio-meet.
 *
 * @param {Object} oldValue - The config {@code Object} which may or may not be
 * in the latest form supported by grommunio-meet and from which a new config
 * {@code Object} is to be constructed if necessary.
 * @returns {Object} A config {@code Object} which is in the latest format
 * supported by grommunio-meet.
 */
...
/**
 * Constructs a new config {@code Object}, if necessary, out of a specific
 * config {@code Object} which is in the latest format supported by grommunio-meet.
 * Such a translation from an old config format to a new/the latest config
 * format is necessary because the mobile app bundles grommunio-meet and
